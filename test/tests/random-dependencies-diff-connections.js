...
module.exports = function(next){
  ...
  var l = conn_number;
  while(l--){
    manager.loadConnection({name:"connection"+l},adapter);
  }

  var num = 100;
  var ari = [["Model0",{},"connection"+Math.floor(Math.random()*conn_number)]];
  l = num-1;
  ...
  while(-1+l--){
    ...
    ari.push(["Model"+l,schema,"connection"+Math.floor(Math.random()*conn_number)]);
  }
  ari.push(["Model"+(num-1),bigSchema,"connection"+Math.floor(Math.random()*conn_number)]);
